...
int my_thread_fun(void *p)
{
	...
	{		
		...
		printk(KERN_INFO "my_thread_fun after sub and test    %lu  Value = %d \n", res,atomic_read(&my_global_variable));
...
static int __init my_simpledriver_init(void)
{
	...
	if(char_thread)
	{
		//printing structure and nested structure values
		printk(KERN_INFO" pid: %d", char_thread->pid);//857 in sched.h
		printk(KERN_INFO" tgid: %d", char_thread->tgid);
		printk(KERN_INFO" Priority: %d", char_thread->prio);
		printk(KERN_INFO" Flags: %ld", char_thread->thread_info.flags);
		printk(KERN_INFO" Status: %ld", char_thread->thread_info.status);
